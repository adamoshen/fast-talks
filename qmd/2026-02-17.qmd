...
## FYI: These deal runs until April 19, 2026
...
## Comparing Fairlife with specialty pizzas with discount
...
large_ppgp <- large_pizzas %>%
  ...
  geom_text(aes(x = price_per_g_protein + 0.05, label = price_label)) +
  scale_fill_identity() +
  theme_minimal() +
...
large_cpr <- large_pizzas %>%
  ...
  mutate(
    ...
    ratio_label = scales::number(calorie_protein_ratio, accurary = 0.01)
  ) %>%
  ggplot(aes(x = calorie_protein_ratio, y = name)) +
  geom_col(aes(fill = bar_fill), show.legend = FALSE) +
  geom_text(aes(x = calorie_protein_ratio + 3, label = ratio_label)) +
  scale_fill_identity() +
  theme_minimal() +
  theme_sub_axis_x(text = element_blank()) +
  theme_sub_axis_y(text = element_text(size = 12)) +
